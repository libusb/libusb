...
CFLAGS="${cflags}" CXXFLAGS="${cflags}" CC="clang" CXX="clang++" ../configure --enable-examples-build --enable-tests-build "$@"
else
...
if [ "${tidy}" = "yes" ]; then
	...
	# add CFLAGS here as configure escapes them
	...
	# Get latest clang-tidy-diff.py script from LLVM project.
