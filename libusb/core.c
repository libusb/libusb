 ...
 */
int API_EXPORTED libusb_get_blockdev_path(libusb_device *dev, int iface_idx, char **path)
{
	...
	*path = NULL;
	...
	return usbi_backend.get_dev_path(dev, iface_idx, USBI_DEV_BLOCK, path);
}
 ...
 * \returns another LIBUSB_ERROR code on error
 */
int API_EXPORTED libusb_get_chardev_path(libusb_device *dev, int iface_idx, char **path)
{
	struct libusb_config_descriptor *config;
	int r;
	...
	*path = NULL;
	...
	return usbi_backend.get_dev_path(dev, iface_idx, USBI_DEV_CHAR, path);
}
