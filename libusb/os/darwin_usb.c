...
static int darwin_fetch_string_descriptor (struct libusb_context *ctx,
  ...
  usb_device_t device = dpriv->device;
   ...
   * able to open every device, so the request is attempted regardless. */
  is_open = ((*device)->USBDeviceOpenSeize (device) == kIOReturnSuccess);

  /* Fetch the supported language IDs (string descriptor 0) and use the
   * first (primary) one for the subsequent request. */
