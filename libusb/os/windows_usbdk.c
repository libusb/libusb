...
static int usbdk_deinitialize_interface(struct libusb_device_handle *dev_handle, uint8_t iface)
{
	UNUSED(dev_handle);
	UNUSED(iface);
	return LIBUSB_SUCCESS;
...
const struct windows_backend usbdk_backend = {
	...
	NULL,  /* usbdk_get_config_string */
	NULL,  /* usbdk_get_interface_string */
	usbdk_open,
	usbdk_close,
