...
#define READ_LE16(p) ((uint16_t)(((uint16_t)((p)[1]) << 8) | \
	((uint16_t)((p)[0]))))

#define READ_LE32(p) ((uint32_t)(((uint32_t)((p)[3]) << 24) | \
	((uint32_t)((p)[2]) << 16) |                              \
	((uint32_t)((p)[1]) << 8) |                               \
	((uint32_t)((p)[0]))))
...
{
	struct libusb_interface_association_descriptor_array *_iad_array = calloc(1, sizeof(*_iad_array));
	int r;
